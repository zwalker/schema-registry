...
public abstract class AbstractKafkaProtobufDeserializer<T extends Message>
  ...
  protected Object deserialize(
    ...
    try {
      ...
      ParsedSchema latestWithMetadata = getLatestWithMetadata(subject);
      if (includeSchemaAndVersion || latestWithMetadata != null) {
      ...
      if (latestWithMetadata != null) {
        migrations = getMigrations(subject, schema, latestWithMetadata);
      }
      ...
      if (!migrations.isEmpty()) {
        ...
        for (int i = 0; i < migrations.size(); i++) {
          ...
          message = (Message) executeRules(
              subject, topic, headers, m.getRuleMode(),
              m.getPrevious(), m.getCurrent(), i == migrations.size() - 1, message
          );
        }
      ...
      message = (Message) executeRules(
          subject, topic, headers, RuleMode.READ, null, schema, true, message
      );
