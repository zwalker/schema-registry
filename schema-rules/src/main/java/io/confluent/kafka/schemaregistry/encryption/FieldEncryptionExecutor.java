...
public class FieldEncryptionExecutor implements FieldRuleExecutor {
  ...
  private boolean keyDeterministic = true;
  ...
  public void configure(Map<String, ?> configs) {
    try {
      ...
      this.dekEncryptCache = CacheBuilder.newBuilder()
          ...
          .build(new CacheLoader<EncryptKey, Dek>() {
            ...
            public Dek load(EncryptKey encryptKey) throws Exception {
              ...
              // Generate new dek
              byte[] rawDek = new Cryptor(dekFormat).generateKey();
      ...
      this.keyCryptor = new Cryptor(getKeyFormat(keyDeterministic));
      this.valueCryptor = new Cryptor(getKeyFormat(valueDeterministic));
    } catch (GeneralSecurityException e) {
  ...
  private static String getKeyFormat(boolean isDeterministic) {
    return isDeterministic ? DETERMINISTIC_KEY_FORMAT : RANDOM_KEY_FORMAT;
  }
  ...
  private byte[] decrypt(RuleContext ctx, byte[] metadata, byte[] ciphertext)
    ...
    Cryptor cryptor = new Cryptor(new String(dekFormat, StandardCharsets.UTF_8));
